  ...
  return <div className="staff-earnings">
    ...
    {loading ? <p>Calculating staff earnings…</p> : <>
      <section className="earnings-summary">
        <article><span>Net service revenue</span><strong>{money.format(totals.reduce((sum, item) => sum + item.service, 0))}</strong></article>
        <article><span>Allocated tips</span><strong>{money.format(totals.reduce((sum, item) => sum + item.tips, 0))}</strong></article>
        <article><span>Unallocated tips</span><strong>{money.format(report.unallocatedTips)}</strong></article>
        <article><span>Net collected</span><strong>{money.format(totals.reduce((sum, item) => sum + item.service + item.tips, 0))}</strong></article>
      </section>
